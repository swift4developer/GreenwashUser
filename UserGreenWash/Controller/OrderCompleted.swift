...
class OrderCompleted: UIViewController,NVActivityIndicatorViewable {
    
    @IBOutlet var tblOfOrderCompletd: UITableView!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.chache = NSCache()
        tblOfOrderCompletd.tableFooterView = UIView()
        
        refreshControl = UIRefreshControl()
        //refreshControl.attributedTitle = NSAttributedString(string: "Pull to refresh")
        refreshControl.addTarget(self, action: #selector(self.refresh(_:)), for: .valueChanged)
        tblOfOrderCompletd.addSubview(refreshControl)
...
extension OrderCompleted : UITableViewDelegate,UITableViewDataSource {
...
extension OrderCompleted {
    func getOrderCompletedList(){
        ...
        if Validation1.isConnectedToNetwork() == true {
            ...
            self.callWSOfgetOrderCompletedList(strURL: strURL, dictionary: dictionary )
        }else {
            ...
            self.tblOfOrderCompletd.addSubview(self.noDataView)
        }
    }
    
    func callWSOfgetOrderCompletedList(strURL: String, dictionary:Dictionary<String,String>){
        let img = UIImage()
                    ...
                    if let data = JSONResponse["bookingHistoryList"] as? [Any] {
                        ...
                        if(self.pageNumber == 1){
                            ...
                            if self.arryOfData.count != 0{
                                DispatchQueue.main.async{
                                    self.tblOfOrderCompletd.reloadData()
                                }
                            }
                        }
                        else{

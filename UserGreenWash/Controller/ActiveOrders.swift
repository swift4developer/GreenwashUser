...
class ActiveOrders: UIViewController,NVActivityIndicatorViewable {

    @IBOutlet var tblOfActiveOrder: UITableView!
    ...
    override func viewDidLoad() {
        ...
        self.chache = NSCache()
        tblOfActiveOrder.tableFooterView = UIView()
        
        refreshControl = UIRefreshControl()
        //refreshControl.attributedTitle = NSAttributedString(string: "Pull to refresh")
        refreshControl.addTarget(self, action: #selector(self.refresh(_:)), for: .valueChanged)
        tblOfActiveOrder.addSubview(refreshControl)
...
extension ActiveOrders : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if type! == "iPad" {
            ...
            cell.lblOfBookingStatus.font = UIFont(name:"Raleway-SemiBold", size:20.0)
        }else {
            if DeviceType.IS_IPHONE_4_OR_LESS || DeviceType.IS_IPHONE_5{
                cell.lblOfBookingStatus.textAlignment = .left
                cell.lblOfBookingStatus.font = UIFont(name:"Raleway-SemiBold", size:10.0)
            }else {
                cell.lblOfBookingStatus.textAlignment = .right
                cell.lblOfBookingStatus.font = UIFont(name:"Raleway-SemiBold", size:12.0)
            }
        }
        
        let currentDic = self.arryOfData[indexPath.row] as! [String : Any]
        ...
        if currentDic["serviceId"] as? String != nil{
            cell.lblOfOrderId.text = "Order Id: " + (currentDic["orderId"] as? String)!
        }else {
        ...
        if currentDic["paymentStatus"] as? String == "Unpaid" {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.falgCallFrm = "ActiveOrder"
        vc.orderId = (currentDic["orderId"] as? String)!
        vc.serviceId = (currentDic["serviceId"] as? String)!
...
extension ActiveOrders {
    func getActiveOrdersList(){
        ...
        strURL = Url.baseURL + "getScheduledOrderList?"
        print(strURL)
        ...
        if Validation1.isConnectedToNetwork() == true {
            ...
            self.callWSOfgetActiveOrdersList(strURL: strURL, dictionary: dictionary )
        }else {
            ...
            self.tblOfActiveOrder.addSubview(self.noDataView)
        }
    }
    
    func callWSOfgetActiveOrdersList(strURL: String, dictionary:Dictionary<String,String>){
        let img = UIImage()
        AFWrapper.requestPostURLForUploadImage(strURL, isImageSelect: false, fileName: "", params: dictionary as [String : AnyObject], image: img, success: { (JSONResponse) in
            ...
            if JSONResponse["status"] as? String == "1"{
                DispatchQueue.main.async {
                    ...
                    if let data = JSONResponse["scheduledOrderList"] as? [Any] {
                        if(self.pageNumber == 1){
                            self.pageNumber =  self.pageNumber + 1;
                            self.arryOfData = data
                            if self.arryOfData.count != 0{
                                DispatchQueue.main.async{
                                    self.tblOfActiveOrder.reloadData()
                                }
                            }
                        }
                        else{
                            self.pageNumber =  self.pageNumber + 1;
                            self.arryOfData =  self.arryOfData + data
                            if self.arryOfData.count != 0{
                                DispatchQueue.main.async{
                                    self.tblOfActiveOrder.reloadData()
                                }
